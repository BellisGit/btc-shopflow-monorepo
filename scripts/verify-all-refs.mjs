...
console.log('🔍 完整验证管理应用构建产物中的所有引用\n');
...
console.log(`🔍 旧 hash 引用: ${foundOldHashes.length}`);
console.log('');

if (foundOldHashes.length > 0) {
  console.log('❌ 发现旧 hash 引用:');
  foundOldHashes.forEach(({ file, hash }, i) => {
    console.log(`  ${i + 1}. ${file}: 包含旧 hash ${hash}`);
  });
  console.log('');
}
...
if (errors.length > 0) {
  ...
  console.log('❌ 构建产物中包含旧 hash 引用，需要修复');
  process.exit(1);
} else {
  console.log('✅ 所有引用都有效！');
  if (warnings.length > 0) {
