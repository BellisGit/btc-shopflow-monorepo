...
if (!command) {
  console.error('Usage: node scripts/run-without-env.js <command> [args...]');
  process.exit(1);
}
...
if (command.includes('/') || command.includes('\\')) {
  ...
  if (!commandPath) {
    ...
    if (fs.existsSync(unixCmd)) {
      ...
      // 如果还是找不到，输出错误信息
      console.error(`[run-without-env] Cannot find command: ${command}`);
      console.error(`[run-without-env] Searched in: ${rootNodeModulesBin}`);
      console.error(`[run-without-env] Current working directory: ${process.cwd()}`);
      process.exit(1);
    }
  }
}
