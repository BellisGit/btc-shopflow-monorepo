...
console.log('🔍 开始验证 admin-app 构建产物...\n');
console.log('   检查目录:', appDistDir);

if (!existsSync(appDistDir)) {
  console.error('❌ 构建产物目录不存在:', appDistDir);
  console.error('   请先运行: pnpm --filter admin-app build');
  process.exit(1);
}

console.log('✅ 构建产物目录存在\n');
...
console.log(`📋 建立 ${fileMap.size} 个文件映射\n`);
...
if (existsSync(indexHtmlPath)) {
  ...
  console.log(`   找到 ${htmlRefs.length} 个引用`);
...
console.log(`   检查了 ${jsFiles.length} 个 JS 文件`);
console.log(`   发现了 ${errors.length} 个错误，${warnings.length} 个警告\n`);

if (errors.length === 0 && warnings.length === 0) {
  console.log('✅ 所有引用验证通过！不会出现 404 错误。\n');
  process.exit(0);
} else {
  if (errors.length > 0) {
    ...
    errors.forEach((error, index) => {
      ...
      console.error(`     引用: ${error.reference}`);
      if (error.expected) {
  ...
  if (warnings.length > 0) {
    ...
    warnings.forEach((warning, index) => {
      console.warn(`  ${index + 1}. 文件: ${warning.file}`);
