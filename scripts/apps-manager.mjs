...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    case 'default':
      ...
      console.log(JSON.stringify(defaultApps, null, 2));
      break;
    
    case 'filter':
      const filtered = parseAppArgs(args);
      const filterPackages = getAppPackageNames(getAppIds(filtered));
      console.log(filterPackages.join(' '));
      break;
    
    default:
      console.log('用法:');
      console.log('  node scripts/apps-manager.mjs list          # 列出所有应用');
      console.log('  node scripts/apps-manager.mjs ids           # 获取所有应用 ID');
      console.log('  node scripts/apps-manager.mjs packages      # 获取所有包名');
      console.log('  node scripts/apps-manager.mjs default        # 获取默认开发应用');
      console.log('  node scripts/apps-manager.mjs filter <ids>  # 过滤应用并返回包名');
      process.exit(1);
  }
}
