...
async function uploadLogo(client) {
  try {
    ...
    if (!stats.isFile()) {
      console.warn('⚠️  logo.png 不存在，跳过');
      return { uploaded: false, skipped: true };
    }
...
async function main() {
  ...
  if (!statSync(iconsSourceDir).isDirectory()) {
    console.error(`❌ 错误：图标目录不存在: ${iconsSourceDir}`);
    process.exit(1);
  }
  ...
  if (cdnMissing.length === 0) {
    console.log('✓ CDN 已存在所有 icons/ 和 logo.png，跳过 OSS 上传');
    return;
  }

  console.log(`⚠️  CDN 缺少 ${cdnMissing.length}/${cdnResults.length} 个文件，将尝试上传 OSS`);
  ...
  console.log('🚀 开始上传图标文件到 OSS...');
  console.log(`   源目录: ${iconsSourceDir}`);
  ...
  try {
    ...
    console.log(`   上传: ${uploadedFiles} 个文件`);
    console.log(`   跳过: ${skippedFiles} 个文件（未变化）`);
    console.log('');
    console.log(`📦 文件可通过 CDN 访问：`);
    console.log(`   ${CDN_DOMAIN}/logo.png`);
    console.log(`   ${CDN_DOMAIN}/icons/`);
  } catch (error) {
    ...
    if (STRICT_UPLOAD) {
      ...
      console.warn(msg);
      console.warn('    将依赖 layout-app 构建产物中的本地 icons/logo 作为后备（避免 404）。');
      return;
    }
  }
}
