...
jsFiles.forEach((filePath) => {
  try {
    ...
    if (hasLoader && hasLoadResourceCall && hasBlobUrl) {
      totalConverted++;
      console.log(`✅ ${relativePath}: 已正确转换`);
    } else if (hasImport) {
...
console.log(`\n📊 检查结果:`);
console.log(`   - 检查的文件数: ${totalChecked}`);
console.log(`   - 已转换的文件数: ${totalConverted}`);
console.log(`   - 未转换的 import() 调用数: ${totalDirectImports}`);
...
if (issues.length > 0) {
  ...
  issues.forEach((issue, index) => {
    ...
    if (issue.hasLoader !== undefined) {
      console.log(`   详情: hasLoader=${issue.hasLoader}, hasLoadResourceCall=${issue.hasLoadResourceCall}, hasBlobUrl=${issue.hasBlobUrl}`);
    }
    console.log('');
  });
  process.exit(1);
  ...
  console.log(`\n验证要点:`);
  console.log(`   ✓ 使用 window.__BTC_RESOURCE_LOADER__ 加载资源`);
  console.log(`   ✓ 支持三级降级：CDN -> OSS -> 本地`);
  console.log(`   ✓ 使用 Blob URL 保持 ES 模块语义`);
  process.exit(0);
}
