...
async function findConfigFiles(dir, configFiles = []) {
  try {
    ...
    if (error.code !== 'EACCES' && error.code !== 'ENOENT') {
      console.warn(`[locale-merge] Error reading directory ${dir}:`, error.message);
    }
  }
...
`;
        ...
        console.warn(`[locale-merge] Failed to load with tsx:`, tsxError.message);
      }
    }
    ...
    // 方法3: 提示安装 tsx
    console.error(`[locale-merge] Cannot load TypeScript file ${filePath}`);
    console.error(`[locale-merge] Please install tsx: pnpm add -D tsx`);
    return null;
  } catch (error) {
    console.error(`[locale-merge] Failed to load config file ${filePath}:`, error.message);
    return null;
  }
}
...
async function processApp(appName) {
  ...
  if (!existsSync(appDir)) {
    console.warn(`[locale-merge] App directory not found: ${appName}`);
    return;
  }

  console.log(`[locale-merge] Processing app: ${appName}`);

  // 查找所有 config.ts 文件
  const configFiles = await findConfigFiles(appDir);
  
  if (configFiles.length === 0) {
    console.log(`[locale-merge] No config.ts files found in ${appName}`);
    return;
  }

  console.log(`[locale-merge] Found ${configFiles.length} config.ts file(s)`);
  ...
  console.log(`[locale-merge] ✅ Generated locale files for ${appName}`);
  console.log(`  - ${zhCNPath}`);
  console.log(`  - ${enUSPath}`);
}
...
main().catch((error) => {
  console.error('[locale-merge] Error:', error);
  process.exit(1);
});
