  ...
  try {
    ...
    try {
      ...
      if (stdout.includes('Found') && stdout.includes('circular dependency') || stdout.includes('Circular') || stdout.includes('circular')) {
        ...
        // 这是解析错误，不是循环依赖，跳过
        console.log(`⚠️  ${label} 存在文件解析错误，跳过循环依赖检测`);
        return { hasCircular: false, cycles: [], skipped: true };
      } else {
        // 其他错误
        console.log(`⚠️  ${label} 检测时遇到问题: ${stderr || execError.message}`);
        return { hasCircular: false, cycles: [], skipped: true };
      }
    }
    
    if (hasCircular) {
      console.error(`❌ ${label} 发现循环依赖:`);
      if (output.trim()) {
    ...
    console.log(`✅ ${label} 未发现循环依赖`);
    return { hasCircular: false, cycles: [] };
  } catch (error) {
    // 捕获其他未预期的错误
    console.log(`⚠️  检查 ${label} 时出错: ${error.message}`);
    return { hasCircular: false, cycles: [], skipped: true };
  }
}
...
function main() {
  ...
  if (!checkMadgeInstalled()) {
    console.log('📦 正在安装 madge...');
    try {
      execSync('pnpm add -D -w madge', { stdio: 'inherit', cwd: rootDir });
      console.log('✅ madge 安装成功\n');
    } catch (error) {
      console.error('❌ 安装 madge 失败，请手动安装: pnpm add -D -w madge');
      process.exit(1);
    }
  }
  ...
  if (circularDirs.length === 0) {
    if (skippedDirs.length > 0) {
      ...
      console.log(`⚠️  注意: ${skippedDirs.length} 个目录因解析错误被跳过（可能是 Vue 文件语法问题）\n`);
    } else {
      console.log('✅ 所有目录均未发现循环依赖！\n');
    }
    process.exit(0);
  } else {
    ...
    circularDirs.forEach(({ dir, label, cycles }) => {
      ...
      if (cycles && cycles.length > 0) {
        ...
        if (cycles.length > 10) {
          console.error(`    ... 还有 ${cycles.length - 10} 个循环依赖`);
        }
      }
    });
    console.error('\n💡 提示: 请修复上述循环依赖后再继续开发\n');
