...
function loadPortConfigs() {
  try {
    ...
    console.error('❌ 无法读取端口配置:', error.message);
    process.exit(1);
  }
}
...
async function main() {
  ...
  for (const result of results) {
    ...
    console.log(`  ${status} ${result.appName.padEnd(20)} :${result.port}`);
  }
  
  console.log('\n');
  
  if (occupiedPorts.length > 0) {
    console.log('⚠️  以下端口已被占用:\n');
          ...
          if (lines.length > 0) {
            ...
            if (pid) {
              try {
                ...
                console.log(`  ❌ ${result.appName} :${result.port}`);
                console.log(`    进程信息: ${processInfo}`);
                if (isSystemService) {
                  console.log(`    ⚠️  这是系统服务进程，无法直接结束`);
                }
              } catch (e) {
                console.log(`  ❌ ${result.appName} :${result.port}`);
    ...
    if (systemServicePorts.length > 0) {
      console.log('⚠️  系统服务占用的端口（需要修改配置或重启系统）:');
      for (const item of systemServicePorts) {
        console.log(`  - ${item.appName} :${item.port} (PID: ${item.pid})`);
      }
      console.log('');
    }
    if (devServerPorts.length > 0) {
      console.log('💡 开发服务器占用的端口，可以使用以下命令结束:');
      for (const item of devServerPorts) {
        if (item.pid) {
          console.log(`  taskkill /F /PID ${item.pid}  # 结束 ${item.appName} (端口 ${item.port})`);
        }
      }
      console.log('');
    }
    console.log('💡 建议: 请先停止占用端口的进程，或使用其他端口。\n');
    process.exit(1);
  } else {
