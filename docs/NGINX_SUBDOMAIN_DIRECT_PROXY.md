...
**前端代码配置：**
- 所有环境统一使用 `baseURL: '/api'`（相对路径）
- 前端代码中**禁止**直接使用 HTTP URL（如 `http://10.0.0.168:8115`）
- 所有 API 请求通过 `/api` 相对路径，由 Nginx 代理到后端

**代理架构：**
- 开发环境：Vite 代理 `/api` → `http://10.80.9.76:8115`
- 生产环境：Nginx 代理 `/api/` → `http://10.0.0.168:8115/api/`

**HTTPS 页面保护：**
- 在 `index.html` 中注入了全局拦截脚本（内联脚本，最早执行）
- 该脚本会拦截所有 HTTP 请求，强制转换为相对路径 `/api`
- 确保在 HTTPS 页面下不会出现 Mixed Content 错误

**禁止事项：**
- ❌ 禁止在代码中硬编码 `http://10.0.0.168:8115` 或任何 HTTP URL
- ❌ 禁止在 `localStorage` 中存储 HTTP URL
- ✅ 统一使用 `/api` 相对路径，由代理处理
