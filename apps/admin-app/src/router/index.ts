...
const routes: RouteRecordRaw[] = [
  ...
  // 系统域路由占位（qiankun会接管，这里只需要Layout）
  {
    path: '/',
    component: Layout,
  ...
  },
  {
    ...
    component: Layout,
    meta: { title: 'Finance App', isSubApp: true }, // 标记为子应用路由
  },
];
...
router.afterEach((to) => {
  ...
  // 检查是否是系统域路径（包括根路径和 /data/* 路径）
  const isSystemPath = to.path === '/' || to.path.startsWith('/data');
  ...
  // 只处理管理域的路由，忽略子应用路由
  // 注意：/admin/* 是管理域路由，不应该被跳过
  // 子应用路由前缀列表（排除 /admin）
  const subAppPrefixes = ['/data', '/logistics', '/engineering', '/quality', '/production', '/finance'];
  // 检查是否是系统域根路径（精确匹配 /）
