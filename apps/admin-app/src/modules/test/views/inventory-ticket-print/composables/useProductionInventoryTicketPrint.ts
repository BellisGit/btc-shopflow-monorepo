    ...
    try {
      ...
      const handleAfterPrint = async () => {
        ...
        if (printDuration < MIN_PRINT_TIME) {
          logger.info('[ProductionInventoryTicketPrint] Print dialog closed too quickly, likely cancelled');
          if (cleanup) {
            cleanup();
          }
          return;
        ...
        try {
          ...
          if (checkTicketPrintService) {
            ...
            logger.warn('[ProductionInventoryTicketPrint] Print service not available, skipping API call');
          }
        } catch (error) {
          // 响应拦截器已显示错误消息，不需要在控制台打印
