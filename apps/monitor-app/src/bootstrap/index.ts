...
const MONITOR_DOMAIN_CACHE_PATH = '@utils/domain-cache';

// 自定义 setupPlugins（monitor-app 需要设置错误捕获）
...
const subAppOptions: SubAppOptions = {
  appId: MONITOR_APP_ID,
  basePath: MONITOR_BASE_PATH,
  domainCachePath: MONITOR_DOMAIN_CACHE_PATH,
  AppComponent: App,
...
export const createMonitorApp = async (props: QiankunProps = {}): Promise<MonitorAppContext> => {
  ...
  if (isStandalone || isUsingLayoutApp) {
    ...
    try {
      ...
      console.warn('[monitor-app] Failed to import domain-cache module:', error);
    }
...
export const mountMonitorApp = async (context: MonitorAppContext, props: QiankunProps = {}) => {
  ...
  // 设置路由同步、事件桥接等
  setupRouteSync(context, MONITOR_APP_ID, MONITOR_BASE_PATH);
  setupHostLocationBridge(context, MONITOR_APP_ID, MONITOR_BASE_PATH);
  setupEventBridge(context);
  ensureCleanUrl(context);

  // 设置退出登录函数（在应用挂载后设置，确保 router 和 i18n 已初始化）
  (window as any).__APP_LOGOUT__ = createLogoutFunction(context, MONITOR_APP_ID);
};
