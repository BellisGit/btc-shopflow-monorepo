...
import type { MonitorAppContext } from './bootstrap';
...
let context: MonitorAppContext | null = null;
  ...
  try {
    ...
    if (context) {
      try {
        ...
        // 卸载失败不影响后续流程，但记录错误
        if (import.meta.env.DEV) {
          console.warn('[monitor-app] 卸载前一个实例失败:', error);
        }
      } finally {
        context = null;
      }
    ...
    console.error('[monitor-app] 渲染失败:', error);
    ...
    try {
      await loadSharedResourcesFromLayoutApp({
        onProgress: (loaded: number, total: number) => {
          if (import.meta.env.DEV) {
            console.log(`[monitor-app] 加载共享资源进度: ${loaded}/${total}`);
          }
        },
      });
    } catch (error) {
      console.warn('[monitor-app] 加载共享资源失败，继续使用本地资源:', error);
...
async function unmount(props: QiankunProps = {}) {
  ...
  if (context) {
    try {
      ...
      // 卸载失败不影响后续流程
      if (import.meta.env.DEV) {
        console.warn('[monitor-app] 卸载失败:', error);
      }
    } finally {
      context = null;
    }
