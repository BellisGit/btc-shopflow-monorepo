...
const render = async (props: QiankunProps = {}) => {
  ...
  try {
    ...
    if (context) {
      try {
        ...
        if (import.meta.env.DEV) {
          console.warn('[dashboard-app] 卸载前一个实例失败:', error);
        }
      } finally {
        context = null;
      }
    ...
    console.error('[dashboard-app] 渲染失败:', error);
    ...
    try {
      await loadSharedResourcesFromLayoutApp({
        onProgress: (loaded: number, total: number) => {
          if (import.meta.env.DEV) {
            console.log(`[dashboard-app] 加载共享资源进度: ${loaded}/${total}`);
          }
        },
      });
    } catch (error) {
      console.warn('[dashboard-app] 加载共享资源失败，继续使用本地资源:', error);
...
async function unmount(props: QiankunProps = {}) {
  ...
  if (context) {
    try {
      ...
      // 卸载失败不影响后续流程
      if (import.meta.env.DEV) {
        console.warn('[dashboard-app] 卸载失败:', error);
      }
    } finally {
      context = null;
    }
...
if (shouldRunStandalone()) {
  ...
  import('./utils/init-layout-app').then(({ initLayoutApp }) => {
      initLayoutApp()
            ...
            waitForViewport().then((viewport) => {
              if (viewport) {
                ...
                render({ container: viewport } as any).then(() => {
                  ...
                  console.error('[dashboard-app] 挂载到 layout-app 失败:', error);
                });
              } else {
                console.error('[dashboard-app] 等待 #subapp-viewport 超时，尝试独立渲染');
                render().catch((error) => {
                  console.error('[dashboard-app] 独立运行失败:', error);
                });
              }
            });
          } else {
        ...
        .catch((error) => {
      console.error('[dashboard-app] 初始化 layout-app 失败:', error);
      ...
      console.error('[dashboard-app] 导入 init-layout-app 失败:', error);
      // 导入失败，直接渲染
      render().catch((error) => {
        console.error('[dashboard-app] 独立运行失败:', error);
    });
