  ...
  try {
    ...
    if (context) {
      try {
        ...
        // 卸载失败不影响后续流程，但记录错误
        if (import.meta.env.DEV) {
          console.warn('[system-app] 卸载前一个实例失败:', error);
        }
      } finally {
        context = null;
      }
    ...
    try {
      await loadSharedResourcesFromLayoutApp({
        onProgress: (_loaded: number, _total: number) => {
          if (import.meta.env.DEV) {
            console.log(`[system-app] 加载共享资源进度: ${_loaded}/${_total}`);
          }
        },
      });
    } catch (error) {
      console.warn('[system-app] 加载共享资源失败，继续使用本地资源:', error);
...
async function unmount(props: QiankunProps = {}) {
  ...
  if (context) {
    try {
      ...
      // 卸载失败不影响后续流程
      if (import.meta.env.DEV) {
        console.warn('[system-app] 卸载失败:', error);
      }
    } finally {
      context = null;
    }
