...
// 从统一配置中获取应用配置
const config = getViteAppConfig('engineering-app');
...
export default defineConfig(({ command, mode }) => {
  ...
  //   - 如果设置了 VITE_PREVIEW=true，使用绝对路径（http://localhost:4185/），用于本地预览测试
  //   - 否则使用相对路径（/），让浏览器根据当前域名（engineering.bellis.com.cn）自动解析
