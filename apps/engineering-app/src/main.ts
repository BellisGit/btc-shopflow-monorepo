...
function render(props: QiankunProps = {}) {
  ...
  router = createRouter({
    ...
    history: createWebHistory(qiankunWindow.__POWERED_BY_QIANKUN__ ? '/engineering' : '/'),
    strict: true,
    routes: [
      {
        path: '/',
  ...
  // 路由守卫：通知主应用添加标签（通过 props，避免幽灵事件）
  if (qiankunWindow.__POWERED_BY_QIANKUN__ && props.setActiveTab) {
    routerUnwatch = router.afterEach((to) => {
      ...
      // ? 通知主应用添加标签（使用完整路径）
      const fullPath = `/engineering${to.path}`;

      // 仍然需要发送事件（但会被命名空间守卫过滤）
