...
async function render(props: QiankunProps = {}) {
  ...
  app.config.errorHandler = (err, instance, info) => {
    ...
    if (err instanceof Error && (
      ...
      if (import.meta.env.DEV) {
        console.warn('[engineering-app] DOM 操作错误已捕获（应用可能正在卸载）:', err.message);
      }
      return;
    }
  };
  ...
  if (container && container instanceof HTMLElement) {
    ...
    if (viewport) {
      ...
      throw new Error('[engineering-app] 使用 layout-app 但未找到 #subapp-viewport 元素');
    }
    ...
    if (viewport) {
      ...
      throw new Error('[engineering-app] qiankun 模式下未提供容器元素且未找到 #subapp-viewport');
    }
  } else {
    // 独立运行模式：使用 #app
