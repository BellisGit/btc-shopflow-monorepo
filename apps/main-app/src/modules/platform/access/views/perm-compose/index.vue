      ...
      <div class="perm-compose-wrap">
        ...
        <div class="perm-compose-left">
          <div class="scope">
            <!-- 头部 -->
            <div class="head">
              <el-text class="label">资源�?/el-text>
              <el-switch
          ...
          <div class="scope">
            ...
            <div class="head">
              ...
              <div class="head-actions" v-if="mode === 'compose'">
                <el-button size="small" text @click="handleSelectAllActions">全�?/el-button>
                ...
                <el-dropdown trigger="click" @command="handleActionTemplate">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      ...
                      <el-dropdown-item command="editor">编辑（查�?编辑�?/el-dropdown-item>
            ...
            <div class="data">
              <el-scrollbar>
                ...
                >
                  ...
                  <el-table-column label="支持�? width="90" align="center" v-if="selectedResources.length > 1">
                    <template #default="{ row }">
            ...
            <div class="data" :style="{ height: mode === 'compose' ? 'calc(100% - 110px)' : 'calc(100% - 40px)' }">
              <el-scrollbar>
                <div class="permission-list">
                  ...
                  <el-empty v-if="composedPermissions.length === 0" description="暂无生成的权�? :image-size="80">
                    <template #image>
                      <el-icon :size="60" color="var(--el-text-color-placeholder)">
                        <Document />
                      </el-icon>
...
// 资源树配�?const resourceTreeRef = ref();
...
// 选择状�?const selectedResources = ref<number[]>([]);
const selectedActions = ref<number[]>([]);
const currentResource = ref<any>(null);

// 矩阵模式状�?const matrixSelections = ref<Set<string>>(new Set());

// 加载状�?const composing = ref(false);
...
const handleModeChange = (val: 'matrix' | 'compose') => {
  message.info(`切换�?{val === 'matrix' ? '矩阵' : '组合'}模式`);
...
// 组合模式：计算实际可生成的权限数（考虑资源约束�?const composeCount = computed(() => {
  ...
  checkedNodes.forEach((resource: any) => {
    selectedActions.value.forEach(actionId => {
      // 检查该资源是否支持该行�?      if (!resource.supportedActions || resource.supportedActions.includes(actionId)) {
        count++;
      }
    });
  });
...
// 资源树复选框变化（两种模式通用�?const handleResourceCheck = (data: any, checked: boolean) => {
  const checkedKeys = resourceTreeRef.value?.getCheckedKeys() || [];
  selectedResources.value = checkedKeys;
};

// 根据当前选择的资源过滤行为（改为并集 + 智能提示�?const filteredActions = computed(() => {
  // 始终显示所有行�?  return actions.value;
});

// 检查行为是否被所有当前资源支持（用于表格行禁�?启用�?const isActionSupported = (actionId: number) => {
...
// 获取行为支持的资源数量（用于提示�?const getActionSupportCount = (actionId: number) => {
...
// 全选行�?const handleSelectAllActions = () => {
...
// 组合模式：生成权限组�?const handleCompose = async () => {
  if (!canCompose.value) {
    message.warning('请先选择资源和行�?);
    return;
  }
...
// 矩阵模式：切换权限（实时生成�?const handleMatrixToggle = (resourceId: number, actionId: number, checked: boolean) => {
...
// 清空所有结�?const handleClearAll = () => {
  composedPermissions.value = [];
  matrixSelections.value.clear();
  message.success('已清空所有权�?);
};
...
// 左侧面板（参�?btc-view-group 样式�?.perm-compose-left {
  width: 300px;
  height: 100%;
...
// 通用 scope 样式（参�?btc-view-group�?.scope {
  display: flex;
  flex-direction: column;
  height: 100%;
  width: 100%;
