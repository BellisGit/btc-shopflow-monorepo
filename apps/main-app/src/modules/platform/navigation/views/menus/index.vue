    ...
    <BtcViewGroup ref="viewGroupRef" :options="viewGroupOptions">
      <template #right>
        <el-tabs v-model="activeTab" class="menu-tabs" style="padding: 10px;">
          <!-- Tab 1: 菜单属�?-->
          <el-tab-pane label="菜单属�? name="props">
...
// 菜单服务（左侧树�?const menuService = service.base.department;
...
// Mock 角色可见�?const roleVisibility = ref([
  { roleName: '系统管理�?, visible: true, reason: '拥有完整权限' },
  { roleName: '部门经理', visible: true, reason: '拥有查看权限' },
  { roleName: '普通员�?, visible: false, reason: '无访问权�? },
]);
...
const viewGroupOptions = reactive({
  ...
  onSelect(menu: any) {
    ...
    // 加载菜单详情
    menuForm.value = { ...menu };
    // 加载已绑定权�?    menuPermissions.value = [1, 2]; // Mock数据
  },
  onEdit(menu?: any) {
...
// 资源表格列（虽然Tab1主要用表单，但保留表格定义以备用�?const resourceColumns = computed<TableColumn[]>(() => [
