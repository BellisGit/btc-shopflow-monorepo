<template>
  <div class="baseline">
    ...
    <el-dialog v-model="compareVisible" title="基线对比" width="80%">
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-tag type="success">新增权限�?</el-tag>
          <el-tag type="danger" style="margin-left: 10px;">删除权限�?</el-tag>
          <el-tag type="warning" style="margin-left: 10px;">修改权限�?</el-tag>
        </el-col>
      </el-row>
...
// Mock数据服务
const baselineService = service.base.department)
    },
    {
      id: 2,
...
const columns = computed<TableColumn[]>(() => [
  ...
  { prop: 'permissionCount', label: '权限�?, width: 100 },
...
const handleFormSubmit = async (data: any, { close, done, next }: any) => {
  try {
    // 自动获取当前权限�?    data.permissionCount = mockHelpers.randomNumber(50, 100);
    await next(data);
...
const handleCreateSnapshot = async () => {
  try {
    await ElMessageBox.prompt('请输入快照名�?, '创建权限快照', {
