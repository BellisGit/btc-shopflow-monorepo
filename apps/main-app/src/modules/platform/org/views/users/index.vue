...
// 部门服务（左侧树�?// 先使�?mock 数据，等待后端提供部门树�?API
  ...
  defaultData: [
    {
      ...
      sortOrder: 1,
      children: [
        { id: 11, name: '仓储�?, deptNameCn: '仓储�?, deptCode: 'WAREHOUSE', sortOrder: 1, parentId: 1 },
        { id: 12, name: '运输�?, deptNameCn: '运输�?, deptCode: 'TRANSPORT', sortOrder: 2, parentId: 1 },
      ]
    },
    {
      id: 2,
      ...
      sortOrder: 2,
      children: [
        { id: 21, name: '生产一�?, deptNameCn: '生产一�?, deptCode: 'PROD1', sortOrder: 1, parentId: 2 },
    ...
    },
    {
      ...
      sortOrder: 3,
      children: [
        { id: 31, name: '质检�?, deptNameCn: '质检�?, deptCode: 'QC', sortOrder: 1, parentId: 3 },
      ]
    },
    {
      id: 4,
    ...
    },
    {
      ...
      sortOrder: 5,
      children: [
        { id: 51, name: '施工�?, deptNameCn: '施工�?, deptCode: 'CONSTRUCTION', sortOrder: 1, parentId: 5 },
      ]
    },
  ]
});
...
const viewGroupOptions = reactive({
  ...
  tree: {
    ...
    // �?高级拖拽规则：不允许拖动一级部门（没有 parentId�?    allowDrag: (node: any) => {
  ...
  onDragEnd(newList: any[]) {
    message.success('部门排序已更�?);
    console.log('新的部门顺序:', newList);
    // 这里应该调用后端API保存新顺�?    // departmentService.updateOrder(newList);
  },
  // �?自定义右键菜单（可选，不配置则使用默认�?  onContextMenu: (item: any) => {
    console.log('右键点击部门:', item.name);
    // 这里可以显示自定义右键菜�?    // 或者返回菜单配置项
  },
  // �?删除部门时的特殊逻辑
  onDelete: (item: any, { next, done }: any) => {
    ElMessageBox.confirm(
      ...
      '提示',
      {
        ...
        cancelButtonText: '转移到上级部�?,
        distinguishCancelAndClose: true,
        type: 'warning'
      }
    )
      .then(() => {
        // 直接删除部门和用�?        next({ ids: [item.id], deleteUsers: true });
      })
      .catch((action: string) => {
        if (action === 'cancel') {
          // 保留用户，转移到上级
...
// 用户表格�?const userColumns = computed<TableColumn[]>(() => [
...
const handleFormSubmit = async (data: any, { close, done, next }: any) => {
  try {
    // 自动添加部门ID和部门名�?    if (selectedDept.value) {
