  ...
  >
    ...
    <template v-else>
      ...
      <el-sub-menu v-show="hasVisibleChildren(menuStructure.main.platform.children)" index="platform">
        ...
        <el-menu-item v-show="isMenuItemVisible('域列表')" index="/platform/domains">
          <el-icon><Location /></el-icon>
          <span>{{ t('menu.platform.domains') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('模块列表')" index="/platform/modules">
          <el-icon><Files /></el-icon>
          <span>{{ t('menu.platform.modules') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('插件列表')" index="/platform/plugins">
      ...
      <el-sub-menu v-show="hasVisibleChildren(menuStructure.main.org.children)" index="org">
        ...
        <el-menu-item v-show="isMenuItemVisible('租户列表')" index="/org/tenants">
          <el-icon><School /></el-icon>
          <span>{{ t('menu.org.tenants') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('部门列表')" index="/org/departments">
          <el-icon><Postcard /></el-icon>
          <span>{{ t('menu.org.departments') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('用户列表')" index="/org/users">
      ...
      <el-sub-menu v-show="hasVisibleChildren(menuStructure.main.access.children)" index="access">
        ...
        <el-menu-item v-show="isMenuItemVisible('资源列表')" index="/access/resources">
        ...
        <el-menu-item v-show="isMenuItemVisible('权限列表')" index="/access/permissions">
          <el-icon><Key /></el-icon>
          <span>{{ t('menu.access.permissions') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('角色列表')" index="/access/roles">
          <el-icon><UserFilled /></el-icon>
          <span>{{ t('menu.access.roles') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('策略列表')" index="/access/policies">
          <el-icon><Document /></el-icon>
          <span>{{ t('menu.access.policies') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('权限组合')" index="/access/perm-compose">
      ...
      <el-sub-menu v-show="hasVisibleChildren(menuStructure.main.navigation.children)" index="navigation">
        ...
        <el-menu-item v-show="isMenuItemVisible('菜单列表')" index="/navigation/menus">
          <el-icon><List /></el-icon>
          <span>{{ t('menu.navigation.menus') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('菜单预览')" index="/navigation/menus/preview">
      ...
      <el-sub-menu v-show="hasVisibleChildren(menuStructure.main.ops.children)" index="ops">
        ...
        <el-menu-item v-show="isMenuItemVisible('操作日志')" index="/ops/audit">
          <el-icon><DocumentCopy /></el-icon>
          <span>{{ t('menu.ops.audit') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('权限基线')" index="/ops/baseline">
          <el-icon><Histogram /></el-icon>
          <span>{{ t('menu.ops.baseline') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('策略模拟器')" index="/ops/simulator">
      ...
      >
        ...
        <el-menu-item v-show="isMenuItemVisible('CRUD测试')" index="/test/crud">
          <el-icon><Tickets /></el-icon>
          <span>{{ t('menu.test_features.crud') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('SVG插件测试')" index="/test/svg-plugin">
          <el-icon><Picture /></el-icon>
          <span>{{ t('menu.test_features.svg') }}</span>
        </el-menu-item>
        <el-menu-item v-show="isMenuItemVisible('国际化测试')" index="/test/i18n">
...
// 判断菜单项是否匹配搜索关键词
const isMenuItemVisible = (text: string) => {
